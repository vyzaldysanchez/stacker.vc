...
contract GaugeD1 {
    ...
    function deposit(uint256 _amountCommitSoft, uint256 _amountCommitHard, address _creditTo) external {
    	...
    	require(fundOpen, "GAUGE: !fundOpen");
    ...
    function upgradeCommit(uint256 _amount) external {
    ...
    function withdraw(uint256 _amount, address _withdrawFor) external {
        ...
        require(_withdrawFor == msg.sender || msg.sender == vaultGaugeBridge, "GAUGE: !bridge for withdrawFor"); // only the bridge contract can use the "withdrawFor" to withdraw for !msg.sender 
    ...
    function sweepCommitSoft() public {
    	require(msg.sender == governance, "GAUGE: !governance");
    	require(block.number > endBlock, "GAUGE: <=endBlock");

        // transfer all remaining ERC20 tokens to the VC address. Fund has closed!
